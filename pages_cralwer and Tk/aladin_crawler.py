...
class AladinCrawler:
    ...
    def crawl(self, keyword):
        ...
        for item in soup.find_all('ul', class_='b_list2'):
            title_tag = item.find('span', class_='b_book_t')
            price_tag = item.find('span', class_='b_price')
            image_tag = item.find('div', class_='flipcover_out')
            ...
            if title_tag and price_tag:
                ...
                if link_tag and link_tag.get('href'):
                    ...
                    if not link.startswith('http'):
                        link = f"https://www.aladin.co.kr/{link}"
                else:
                    link = '링크 없음'
